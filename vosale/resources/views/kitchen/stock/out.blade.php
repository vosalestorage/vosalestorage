<!DOCTYPE html><html lang="id"><head><meta charset="UTF-8"><title>Barang Keluar Kitchen</title>
...
<a href="/kitchen/stock/out" class="nav-link active"><i class="bi bi-arrow-up-circle"></i> Barang Keluar</a>
...
<div class="topbar"><div><h5 class="mb-0 fw-bold">Barang Keluar — Kitchen</h5><small class="text-muted">Catat penggunaan stok</small></div></div>
...
<h6 class="fw-bold mb-3"><i class="bi bi-arrow-up-circle text-danger me-2"></i>Catat Barang Keluar</h6>
<form method="POST" action="/kitchen/stock/out">
@csrf
...
@foreach($items as $item)<option value="{{ $item->id }}" data-stock="{{ $item->stock }}" data-unit="{{ $item->unit }}" {{ old('item_id')==$item->id?'selected':'' }}>{{ $item->name }} (Stok: {{ $item->stock }} {{ $item->unit }})</option>@endforeach
</select></div>
...
<div class="col-md-3"><label class="form-label fw-semibold">Keterangan</label><input type="text" name="note" class="form-control" placeholder="contoh: Digunakan masak" value="{{ old('note') }}"></div>
<div class="col-md-3">
<div id="stock_info" class="stock-info mb-2 d-none">Stok tersedia: <strong id="current_stock" class="text-success">0</strong> <span id="unit_label"></span></div>
<button type="submit" class="btn btn-danger w-100"><i class="bi bi-dash-circle me-1"></i> Kurangi Stok</button>
</div>
</div>
</form>
</div>
<div class="card card-main">
<div class="card-header bg-white border-0 pt-4 px-4"><h6 class="fw-bold mb-0">Riwayat Barang Keluar</h6></div>
...
<td><span class="badge bg-danger">-{{ $trx->quantity }} {{ $trx->item->unit ?? '' }}</span></td>
<td>{{ $trx->stock_before }}</td><td>{{ $trx->stock_after }}</td>
<td>{{ $trx->note ?? '-' }}</td><td>{{ $trx->user->name ?? '-' }}</td>
</tr>
@empty
<tr><td colspan="7" class="text-center py-5 text-muted"><i class="bi bi-inbox fs-1 d-block mb-2"></i>Belum ada riwayat barang keluar</td></tr>
@endforelse
</tbody>
</table>
</div>
...
<script>
function updateStockInfo(select){const o=select.options[select.selectedIndex];const info=document.getElementById('stock_info');const stockEl=document.getElementById('current_stock');if(select.value){stockEl.textContent=o.dataset.stock;stockEl.className=parseInt(o.dataset.stock)<=5?'text-danger fw-bold':'text-success fw-bold';document.getElementById('unit_label').textContent=o.dataset.unit;info.classList.remove('d-none');document.getElementById('quantity').max=o.dataset.stock;}else{info.classList.add('d-none');}}
